...
<template>
  ...
  <aside
    class="fixed inset-y-0 left-0 z-40 flex h-screen w-72 shrink-0 flex-col border-r border-[#c49a4d] bg-[#e7c982] text-[#211b12] shadow-[0_30px_80px_-30px_rgba(120,82,25,0.45)] transition-transform duration-200 lg:sticky lg:top-0 lg:z-auto lg:translate-x-0"
    :class="abierto ? 'translate-x-0' : '-translate-x-full'"
  >
    <div class="border-b border-[#c49a4d] bg-gradient-to-br from-[#f3dca0] via-[#e7c982] to-[#d5ae5f] px-5 py-5">
      <div class="flex items-center gap-3">
        <div
          class="flex h-11 w-11 items-center justify-center overflow-hidden rounded-xl bg-white/80 ring-1 ring-[#b98d42]/35"
        >
          <img :src="marca.logoSrc" alt="Logo" class="h-8 w-8 object-contain" />
        </div>
        <div class="min-w-0 flex-1">
          <p class="truncate text-base font-semibold tracking-tight text-[#211b12]">{{ marca.nombre.value }}</p>
          <p class="mt-0.5 truncate text-[11px] italic text-[#71501d]">{{ marca.slogan.value }}</p>
        </div>
        <UButton
          ...
          variant="ghost"
          size="sm"
          class="shrink-0 text-[#211b12] lg:hidden"
    ...
    <nav class="flex-1 space-y-5 overflow-y-auto py-4">
      <div v-for="seccion in secciones" v-show="seccion.items.length" :key="seccion.titulo" class="px-2">
        <p class="mb-2 px-3 text-[10px] font-semibold uppercase tracking-[0.18em] text-[#735622]">
          {{ seccion.titulo }}
        </p>
        <div class="space-y-1">
          <NuxtLink
            ...
            :key="item.to"
            :to="item.to"
            class="group flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-[#2d2417] transition-all duration-200 hover:bg-[#f5e5b8] hover:text-[#6f4d17]"
            active-class="bg-[#fff8e8] text-[#6f4d17] shadow-[0_8px_18px_-12px_rgba(96,64,12,0.8)] ring-1 ring-inset ring-[#b98d42]/50"
          >
            <span
              class="flex h-8 w-8 items-center justify-center rounded-lg bg-[#d6b267] text-[#60461b] transition group-hover:bg-[#fff0c8] group-hover:text-[#6f4d17]"
            >
    ...
    <div class="border-t border-[#c49a4d] bg-[#d6b267] px-4 py-4">
      <p class="truncate text-sm font-medium text-[#211b12]">{{ auth.usuario?.email }}</p>
      <div class="mt-2 flex items-center justify-between gap-2">
        <p class="text-[10px] uppercase tracking-[0.2em] text-[#735622]">{{ auth.rol }}</p>
