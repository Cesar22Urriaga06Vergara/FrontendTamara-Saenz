...
//#region pages/codeudores/index.vue?vue&type=script&setup=true&lang.ts
...
var index_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	setup(__props) {
		...
		const codeudorEditando = ref(null);
		...
		const codeudorParaBaja = ref(null);
		async function cargar() {
			cargando.value = true;
			try {
				if (busqueda.value) {
					codeudores.value = await useApiFetch("/codeudores/buscar", { params: {
						documento: busqueda.value,
						nombre: busqueda.value
					} });
					total.value = codeudores.value.length;
				} else {
					const data = await useApiFetch("/codeudores", { params: {
						page: page.value,
						limit: limit.value
					} });
		...
		function abrirEdicion(codeudor) {
		...
		async function guardar() {
			...
			try {
				if (codeudorEditando.value) await useApiFetch(`/codeudores/${codeudorEditando.value.id}`, {
					method: "PATCH",
					body: formulario
				});
				else await useApiFetch("/codeudores", {
					method: "POST",
					body: formulario
				});
		...
		function confirmarBaja(codeudor) {
		...
		async function ejecutarBaja() {
			...
			try {
				await useApiFetch(`/codeudores/${codeudorParaBaja.value.id}`, {
					method: "PATCH",
					body: { activo: false }
				});
		...
		async function alternarActivo(codeudor) {
			error.value = "";
			try {
				await useApiFetch(`/codeudores/${codeudor.id}`, {
					method: "PATCH",
					body: { activo: true }
				});
				await cargar();
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(`<div${ssrRenderAttrs(_attrs)}><div class="flex items-center justify-between mb-4"><h1 class="text-xl font-semibold text-slate-900">Codeudores</h1>`);
			_push(ssrRenderComponent(_component_UButton, {
				...
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) _push(`Nuevo codeudor`);
					else return [createTextVNode("Nuevo codeudor")];
				}),
				_: 1
			}, _parent));
			_push(`</div>`);
						...
						_push(ssrRenderComponent(_component_UTable, {
							...
							"empty-state": withCtx((_, _push, _parent, _scopeId) => {
								if (_push) {
									...
									_push(`<p${_scopeId}>No hay codeudores que coincidan con la búsqueda.</p></div>`);
								} else return [createVNode("div", { class: "text-center py-10 text-slate-400" }, [createVNode(_component_UIcon, {
									name: "i-heroicons-user-plus",
									class: "w-10 h-10 mx-auto mb-2"
								}), createVNode("p", null, "No hay codeudores que coincidan con la búsqueda.")])];
							}),
							_: 1
						}, _parent, _scopeId));
						if (!unref(busqueda)) {
				...
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) _push(ssrRenderComponent(_component_UCard, null, {
						header: withCtx((_, _push, _parent, _scopeId) => {
							if (_push) _push(`<p class="font-semibold text-slate-900"${_scopeId}>${ssrInterpolate(unref(codeudorEditando) ? "Editar codeudor" : "Nuevo codeudor")}</p>`);
							else return [createVNode("p", { class: "font-semibold text-slate-900" }, toDisplayString(unref(codeudorEditando) ? "Editar codeudor" : "Nuevo codeudor"), 1)];
						}),
						footer: withCtx((_, _push, _parent, _scopeId) => {
							if (_push) {
								...
								_push(ssrRenderComponent(_component_UButton, {
									...
									default: withCtx((_, _push, _parent, _scopeId) => {
										if (_push) _push(`${ssrInterpolate(unref(codeudorEditando) ? "Guardar cambios" : "Crear codeudor")}`);
										else return [createTextVNode(toDisplayString(unref(codeudorEditando) ? "Guardar cambios" : "Crear codeudor"), 1)];
									}),
									_: 1
								}, _parent, _scopeId));
								_push(`</div>`);
								...
								default: withCtx(() => [createTextVNode(toDisplayString(unref(codeudorEditando) ? "Guardar cambios" : "Crear codeudor"), 1)]),
								_: 1
							}, 8, ["loading", "disabled"])])];
						}),
						default: withCtx((_, _push, _parent, _scopeId) => {
						...
						header: withCtx(() => [createVNode("p", { class: "font-semibold text-slate-900" }, toDisplayString(unref(codeudorEditando) ? "Editar codeudor" : "Nuevo codeudor"), 1)]),
			...
			_push(ssrRenderComponent(_component_UiConfirmModal, {
				...
				title: "Dar de baja",
				message: `¿Confirma dar de baja a ${unref(codeudorParaBaja)?.nombreCompleto}?`,
...
//#endregion
//#region pages/codeudores/index.vue
var _sfc_setup = index_vue_vue_type_script_setup_true_lang_default.setup;
index_vue_vue_type_script_setup_true_lang_default.setup = (props, ctx) => {
	const ssrContext = useSSRContext();
	(ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/codeudores/index.vue");
	return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};
var codeudores_default = index_vue_vue_type_script_setup_true_lang_default;

export { codeudores_default as default };
//# sourceMappingURL=codeudores-CwsKsABQ.mjs.map
